...
namespace OutlookDemo.DiaLog
{
    public partial class FormClassSection : Form
    {
            ...
            try
            {
                using (SqlConnection connection = ketNoi.MoKetNoi())
                {
                    ...
                    if (state == 0)
                    {
                        if (ClassSectionCode == "")
                        {
                            query = "INSERT INTO [dbo].[class_section] ([class_section_id],[class_section_name],[number_credits],[class_section_code],[theory],[practice],[elective],[semester],[type]) VALUES(NEWID(),@class_section_name,@number_credits,@class_section_code,@theory,@practice,@elective,@semester,@type)";

                        }
                        else
                        {
                            query = "UPDATE [dbo].[class_section] SET [class_section_name] = @class_section_name,[number_credits] = @number_credits,[class_section_code] = @class_section_code ,[theory] = @theory,[practice] = @practice,[elective] = @elective,[semester] = @semester,[type] = @type WHERE [class_section_id] = @class_section_id;";
                        }
                    }
                    else
                    {
        ...
        {
            DialogResult dialogResult = MessageBox.Show($"Bạn có muốn xóa học phần '{txtName.Text}' không?", "Xóa học phần", MessageBoxButtons.YesNo);
